        ...
        foreach($category as $cat){
          ...
          $SelectR = $systemcore->SelectCustomize("SELECT * FROM vims_report WHERE 2nd_dose = '$dose_2_report' AND 1st_dose = '$dose_1_report' AND vaccine_name = '$vaccine_name' ORDER BY lastname ASC");
            foreach($SelectR as $value){ $x++; 
              ...
              if($v_brgy == $brgy_1 && $value["category"] == $cat && $month_of_vaccination == $report_month_vaccination){ 
                $SelectD = $systemcore->SelectCustomize("SELECT * FROM vims_report WHERE firstname = '$firstname' AND middlename = '$middlename' AND lastname = '$lastname' AND 2nd_dose = 'Y' ORDER BY lastname ASC");
                if($SelectD == "none"){
                  $error = true;    
                }
              }   
