<?php
    ...
    $id = $_GET['primary_id'];
    $id_p = $_GET['primary_id'];

    $SelectGroups = $systemcore->SelectTable("local_data_fetcher WHERE id = '$id'");
    ...
    // echo $qr_id;
    $SelectGroups = $systemcore->SelectTable("vaccine_registration WHERE lastname = '$lastname' AND firstname = '$firstname' AND middlename = '$middlename'");
    if($SelectGroups != "none"){
        foreach($SelectGroups as $value){
            $id = $value["id"];
            $qr_id = $value["qr_id"];
            ...
            // Second dose vaccine
            $sec_vaccine_name = $value["sec_vaccine_name"] ?? "N/A"; // Check if null or empty
            $sec_batch_number = $value["sec_batch_number"] ?? "N/A";
            $sec_lot_number = $value["sec_lot_number"] ?? "N/A";
            $sec_time_stamp = $value["sec_date_of_vaccination"] ?? "N/A";
...
<div class="modal-body p-2 m-0"  style=" height: 65vh;  overflow-y: auto;" >
    <form id ="update_form" enctype="multipart/form-data" class="p-0 m-0">
        <input type="text" class="form-control float-right" value="<?php echo $id_p;?>" id="primary_key" name="primary_key" hidden >
        ...
        <div class="card" style="background-color:rgb(239, 240, 206)">
            <div class="col-12 row  pr-3 pl-3">
                ...
                <div class="col-12 row"><!-- Vaccination: -->
                    ...
                    <!-- 2nd vaccine start -->
                    <div class="form-group col-sm-3">
                        <input type="date" class="form-control regis_form  bg-info" name="sec_date_of_vaccination" value="<?php echo $sec_time_stamp; ?>" alt="required">
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <option value="<?= $sec_vaccine_name; ?>" hidden><?= $sec_vaccine_name; ?></option>
                        <option value="N/A" >N/A</option>
                            <?php foreach($Global_vaccine_name as $data){?>
                                <option value="<?php echo $data; ?>"><?php echo $data; ?></option>
                            <?php 
                    ...
                    <div class="form-group col-sm-3">
                        <input type="text" class="form-control regis_form" name="sec_batch_number" value="<?= $sec_batch_number ?>">
                        <label for="exampleInputEmail1">Batch Number?<span style="color:red;">*</span></label>
                    </div>

                    <div class="form-group col-sm-3">
                        <input type="text" class="form-control regis_form" name="sec_lot_number" value="<?= $sec_lot_number ?>">
                        <label for="exampleInputEmail1">Lot Number?<span style="color:red;">*</span></label>
                    </div>
                    <!-- 2nd vaccine end -->
